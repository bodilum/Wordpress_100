...
const animEnterSingleOCBWork = (container) => {
  ...
  .fromTo("#mainHeader ._mainNavs", { y: 0, opacity: 1, display: 'block' }, 
  { duration: 1, y: 20, ease: "power4.out", opacity: 0, display: 'none', stagger: { amount: 0.3 } }, "-=1.2")
  ...
  console.log('Just played anim enter single ocbwork animation');
...
const animLeaveSingleOCBWork = (container, done) => {
  const tl1 = gsap.timeline();

  tl1
  // .fromTo('#mainHeader ._extraFooterInfos ._body ._center ._visitSiteBtn', { display: 'block', opacity: 1, y: 0 }, 
  // { duration: 2, ease: 'power4.out', display: 'none', opacity: 0, y: -100 })
  .fromTo('#mainHeader ._extraFooterInfos ._body ._center ._exploreBtn', { display: 'none', opacity: 0, y: 200 }, 
  { duration: 2, ease: 'power4.out', display: 'block', opacity: 1, y: 0 })
...
let curPgObj, nextPgObj;
 ...
 barba.hooks.beforeLeave(({ current, next, trigger }) => {
  if(maxiMainNavMode) isSingleWorkPost = true;
  ...
  barba.hooks.beforeEnter(({ current, next }) => {
    ...
    if(allCatNames.includes('ocbsa works')) {
      const tl1 = gsap.timeline();

      tl1
      .set('#ocbSingleWorkSwiper', { display: 'block' })
      .set('#ocbSingleWorkSwiper .allSlides .slide', { y: dh * 1.2 })
      .set('#backToProject', { y: -150, display: 'block', opacity: 1 })
      // .set('#visitSiteBtn', { y: 250, display: 'block', opacity: 1 })
      // .set('#ocbSingleWorkSwiperSideBar', { y: dh, display: 'block', opacity: 0 })
      ;
      ...
      $("#ocbSingleWorkSwiper").css({ width: dw, height: dh });
      $("#ocbSingleWorkSwiperSideBar ._holder").css({ height: dh * 0.6 });
      $("#ocbSingleWorkSwiper .allSlides").css({ left: allSlidesLeft + 'px' });
      $("#ocbSingleWorkSwiper .allSlides .slide").css({ width: slideWidth + 'px', height: slideHeight + 'px' });


      // get first slide image
      // let slideImg1 = $("#ocbSingleWorkSwiper .allSlides .slide1")

      let initSlideInd = 0;
      
      let slideImg1_url = $("#ocbSingleWorkSwiperSideBar ._thumb[uid='" + initSlideInd + "']").attr('imgurl');

      if(slideImg1_url) {
        // console.log('SlideImg Url: ', slideImg1_url);
        $("#ocbSingleWorkSwiper .allSlides .slide1").css({ 'background-image': `url(${ slideImg1_url })` });
      }
      ...
      const sideBarSwiper = new Swiper('.sideBarSwiper', {
        ...
        on: {
          ...
          scroll: function(swpr, ev) {
            ...
            $("#ocbSingleWorkSwiperSideBar ._highlighter").css({ opacity: 0.2 });
            if(mouseSideBarScrollTimeout1) clearTimeout(mouseSideBarScrollTimeout1);

            mouseSideBarScrollTimeout1 = setTimeout(() => {
              scrollHighlighterToActiveSlide(swpr);
              $("#ocbSingleWorkSwiperSideBar ._highlighter").css({ opacity: 1 });
      ...
      function scrollHighlighterToActiveSlide(swpr) {
        ...
        $("#ocbSingleWorkSwiperSideBar ._highlighter").css({ top: curSlideTop });
      }
...
//           return;
        ...
        async afterEnter(data) {  movNavXVals(boundsRightX); },
...
$(document).on('click', '#mainMediaNavMoreInfo .backToHome', function() {
  ...
  /////// MainNavs
  $('#mainMediaNav ._mainContents.' + pgNavMainClass + ' ._itmCard, #mainHeader ._mainNavs ._itm1').removeClass('_maxi');
  $('#mainMediaNav ._mainContents.' + pgNavMainClass + ' ._itmCard, #mainHeader ._mainNavs ._itm1').addClass('_mini _canHover');
...
$(document).on('click', '#mainMediaNav ._mainContents._featuredWorks ._itmCard._mini', function() {
  ...
  if(mainNavActivCat !== '') {
    catid = $(this).attr('catid'); 
    activMaxiNavEl = $(`#mainMediaNav ._mainContents._featuredWorks ._itmCard.${mainNavActivCat}:eq(${catid})`);
    activMiniMaxiNavEl = $(`#mainHeader ._mainNavs ._itm1.${mainNavActivCat}:eq(${catid})`);
  } else {
    catid = $(this).index();
    activMaxiNavEl = $(`#mainMediaNav ._mainContents._featuredWorks ._itmCard:eq(${catid})`);
  ...
  $('#mainMediaNav ._mainContents._featuredWorks ._itmCard').css({ 'width' : navWidth + 'px' });
  ...
  $('#mainMediaNav ._mainContents._featuredWorks ._itmCard, #mainHeader ._mainNavs ._itm1').removeClass('_mini _canHover');
  $('#mainMediaNav ._mainContents._featuredWorks ._itmCard, #mainHeader ._mainNavs ._itm1').addClass('_maxi');
