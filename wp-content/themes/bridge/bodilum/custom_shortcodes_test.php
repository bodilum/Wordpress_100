...
function showMiniNavsFunc($atts, $content = null) {
    $cat = 'featured work'; // all featured works category by default 
    $cat1 = 'ocbsa works'; // all works
    $cat2 = 'ocbsa creator'; // creators
    ...
    $qpostsAllWorks = new WP_Query(array(
        'category_name' => $cat1,
        'post_status' => 'publish',
        'post_type' => 'post',
        'posts_per_page' => $post_count,
    ...
    $qpostsCreators = new WP_Query(array(
        'category_name' => $cat2,
        'post_status' => 'publish',
        'post_type' => 'post',
        'posts_per_page' => $post_count,
    ...
    $posts = $qposts->get_posts();
    $postsAllWorks = $qpostsAllWorks->get_posts();
    $postsCreators = $qpostsCreators->get_posts();
    ...
    foreach( $postsAllWorks as $post ) { 
        $cat_classes = catClassesStr($post->ID);
        $activClass = ( $postCount1 == 0) ? '_prevActiv' : '';
        $qryStr1 .= '<div class="_itm1 _mini ' . $activClass . ' ' . $cat_classes . '" uid="' . $postCount1 . '"></div>';
        $postCount1++;
        }
    foreach( $postsCreators as $post ) { 
        $cat_classes = catClassesStr($post->ID);
        $activClass = ( $postCount2 == 0) ? '_prevActiv' : '';
        $qryStr2 .= '<div  class="_itm1 _mini ' . $activClass . ' ' . $cat_classes . '" uid="' . $postCount2 . '"></div>';
        $postCount2++;
        }

        /// Extra Footer Info here...
    $extraFooterInfo = '
    <div class="_extraFooterInfos">
        <div class="_body">
            <div class="_left">
                <div class="_gridContent1">
                    <div class="_titles">
                        <div class="_itm"><span>Complete</span></div>
                        <div class="_itm"><span>Type</span></div>
                        <div class="_itm"><span>Role</span></div>
                        <div class="_itm"><span>Client</span></div>
                    </div>
                    <div class="_content">
                        <div class="_itm"><span>April 2022</span></div>
                        <div class="_itm"><span>Corporate Ad</span></div>
                        <div class="_itm"><span>Director of Photography</span></div>
                        <div class="_itm"><span>Standard Bank</span></div>
                    </div>
                </div>
            </div>
            <div class="_center">
                <a href="" class="_exploreBtn">
                    <div class="_txt1"><span><b>Explore</b></span></div>
                    <div class="_line"><span></span></div>
                    <div class="_arrow"><span><b>ꜜ</b></span></div>
                </a>                
            </div>
            <div class="_right">
                <div class="_itm"><span>The global economic network of seamlessly interconnected businesses</span></div>
            </div>
        </div>
    </div>';

    return '
        <div class="_mainNavs _featuredWorks">
            ' . $qryStr . '
        </div>
        <div class="_mainNavs _allWorks">
            ' . $qryStr1 . '
        </div>
        <div class="_mainNavs _allCreators">
            ' . $qryStr2 . '
        </div>
...
function mainHomeNavFunc2() {
    ...
    /// setting up the category button section
    $catStr = "";

    foreach( $mainCatSelArr as $ct ) {
        $trimmed = trim($ct);
    ...
    foreach( $posts as $post ) { 
        ...
        $workrole = get_field('role', $post->ID);
        $type = get_field('type', $post->ID);
        $completed = get_field('completed_date', $post->ID);
        $workdesc = get_field('portfolio_desc', $post->ID);
        $workdesc = ( trim($workdesc) != '') ? substr( $workdesc, 0, 60 ) . "..." : '';
        ...
        $qryStr1 .= "<div url='" . $permalink . "' class='_itmCard _mini _canHover " . $activClass . " " . $cat_classes . " swiper-slide' 
        uid='" . $postCount . "' fgcolor='" . $fgColor . "' bgcolor='" . $bgColor . "' client='" . $client . "' type='" . $type . "' workrole='" . $workrole . "' completed='" . $completed . "' workdesc='" . $workdesc . "'>
            <div class='_itm'>
                <a>
                    <span class='_img' style='background-image: url(" . $image[0] . ")'></span>
                </a>    
    ...
    foreach( $postsAllWorks as $post ) { 
        ...
        $qryStr1_1 .= "<div url='" . $permalink . "' class='_itmCard _mini _canHover " . $activClass . " " . $cat_classes . " swiper-slide' 
        uid='" . $postCount . "' fgcolor='" . $fgColor . "' bgcolor='" . $bgColor . "' client='" . $client . "' type='" . $type . "' workrole='" . $workrole . "' completed='" . $completed . "' workdesc='" . $workdesc . "'>
            <div class='_itm'>
                <a>
                    <span class='_img' style='background-image: url(" . $image[0] . ")'></span>
    ...
    foreach( $postsCreators as $post ) { 
        ...
        $team_mem_pos = get_field('team_member_position', $post->ID, true);
        ...
        $qryStr1_2 .= "<div url='" . $permalink . "' class='_itmCard _mini _canHover " . $activClass . " " . $cat_classes . " swiper-slide' 
        uid='" . $postCount . "' fgcolor='" . $fgColor . "' bgcolor='" . $bgColor . "' client='" . $client . "' type='" . $type . "' workrole='" . $workrole . "' completed='" . $completed . "' workdesc='" . $workdesc . "'>
            <div class='_itm'>
                <a>
                    <span class='_img' style='background-image: url(" . $image[0] . ")'></span>
            ...
            <div class='_info2'>
                <div class='_txt1'>" . $ptitle . "</div>
                <div class='_txt2'>" . $team_mem_pos . "</div>
            </div>
        </div><!-- end of _itmCard -->";
        $postCount++;
    }
    ...
    <div id='mainMediaNavMoreInfo'>
        ...
        <div class='bigTxt _featuredWorks'>" . $allBigTxt . "</div>
        <div class='bigTxt _allWorks'>" . $allBigTxt_1 . "</div>
        <div class='bigTxt _allCreators'>" . $allBigTxt_2 . "</div>
    ...
    <div id='mainMediaNav'>
        ...
        <div class='_mainContents swiper _featuredWorks'>                  
            <div class='swiper-wrapper'>
                " . $qryStr1 . "
            </div>
        </div>
        <div class='_mainContents swiper _allWorks'>                  
            <div class='swiper-wrapper'>
                " . $qryStr1_1 . "
            </div>
        </div>
        <div class='_mainContents swiper _allCreators'>                  
            <div class='swiper-wrapper'>
                " . $qryStr1_2 . "
            </div>
        </div>
